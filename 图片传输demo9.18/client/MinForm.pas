...
procedure TMintForm.SendScreen;
var
  psize: Integer;
  count: Integer;
  i: Integer;
...
begin
  ...
  BitBlt(BitMap.Canvas.Handle, 0, 0, BitMap.Width, BitMap.Height, sdc, 0, 0, SRCCOPY);
  //StretchBlt(ScreenFor.Canvas.Handle, 0, 0, 300, 300, sdc, 0, 0, 300, 300, SRCCOPY);
  ReleaseDC(shwnd, sdc);
  PStream := TMemoryStream.Create;
  BitMap.SaveToStream(PStream);
  psize := PStream.Size;
  count := PStream.Size div 1024;
  ...
  begin
    PStream.read(ScreenMessage.Pbuf, 1024);
    send(client.Socked, ScreenMessage, 1036, 0);
  end;
  if (psize mod 1024) <> 0 then
  begin
    FillMemory(@ScreenMessage.Pbuf[0], 1024, 0);
    pMod := psize mod 1024;
