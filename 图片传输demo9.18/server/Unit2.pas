...
uses
  ...
  ZAbstractConnection, ZConnection, Unit1;
...
//            send(clntSock, Pointer(str)^, Length(str), 0);
        ...
        begin
          recvReturn := recv(fdsetOld.fd_array[I], buf, 1036, 0);
...
//                userInfo := CheckSqlForRegeister(R_C_Regeister);
            ...
            C_Screen:
              begin
                CopyMemory(@ScreenMsg, @buf[0], recvReturn);
                CStream.Write(ScreenMsg.Pbuf, 1024);
                maxcount := ScreenMsg.head.Size div 1024;
                Inc(count);
                if count = maxcount then
                begin
                  if (ScreenMsg.head.Size mod 1024) = 0 then
                  begin
                    GetScreen;
                  end;
                end
