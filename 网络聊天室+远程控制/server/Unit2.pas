...
type
  EventOperator = (NoEvent, MouseDownEvent, MouseUpEvent, MouseMoveEvent, KeyDownEvent, KeyUpEvent);
...
var
  ...
  RcontrolMsgMouse: C_RControl_MOUSE;
  RcontrolMsgKey: C_RControl_KEY;
...
procedure SendMouseControl(msg: C_RControl_MOUSE);
var
  desSocket: TSocket;
  i: Integer;
begin
  ...
  msg.head.Operation := S_CONTROL_MOUSE;
  send(desSocket, msg, SizeOf(msg), 0);
end;

procedure SendKeyControl(msg: C_RControl_KEY);
var
  desSocket: TSocket;
  i: Integer;
begin
  ...
  msg.head.Operation := S_CONTROL_KEY;
  send(desSocket, msg, SizeOf(msg), 0);
end;
...
//                userInfo := CheckSqlForRegeister(R_C_Regeister);
            ...
            C_CONTROL_MOUSE:
              begin
                CopyMemory(@RcontrolMsgMouse, @buf[0], 24);
                FillMemory(@buf, 32836, 0);
                SendMouseControl(RcontrolMsgMouse);
              end;
            C_CONTROL_KEY:
              begin
                CopyMemory(@RcontrolMsgKey, @buf[0], SizeOf(C_RControl_KEY));
                FillMemory(@buf, 32836, 0);
                SendKeyControl(RcontrolMsgKey);
              end;
