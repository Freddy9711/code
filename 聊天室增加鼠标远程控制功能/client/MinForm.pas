...
type
  ...
  public
    ...
    procedure RecvInfo;
    procedure GetScreen;
    procedure ParseControlInfo(RecvControl: S_RControl);
  private
    FEvent: TEvent;
    FStarted: Boolean;
  public
...
var
  ...
  RecvControl: S_RControl;
  FControlScreen: TControlScreen;
  EventMessage: C_RControl;
...
procedure TReadUserInfo.ParseControlInfo(RecvControl: S_RControl);
begin
...
begin
  ...
  else if headmes.Operation = S_CONTROL then
  begin
    CopyMemory(@RecvControl, @recvmes[0], RecvSize);
    ParseControlInfo(RecvControl);
  end;
//  Application.ProcessMessages;
end;
...
begin
  ...
  begin
    ...
    begin
      EventMessage.head.Flag := PACKAGE_FLAG;
      EventMessage.head.Size := 24;
      EventMessage.head.Operation := C_CONTROL;
      EventMessage.PointX := PosX;
      EventMessage.PointY := PosY;
      EventMessage.EventType := MouseDownEvent;
      EventMessage.FMouseButton := MouseButton;
      send(client.socked, EventMessage, SizeOf(EventMessage), 0);
      SendEvent := NoEvent;
    end
    ...
    begin
      ...
      EventMessage.PointX := PosX;
      EventMessage.PointY := PosY;
      EventMessage.EventType := MouseUpEvent;
      EventMessage.FMouseButton := MouseButton;
      send(client.socked, EventMessage, SizeOf(EventMessage), 0);
      SendEvent := NoEvent;
    end
    ...
    begin
      ...
      EventMessage.PointX := PosX;
      EventMessage.PointY := PosY;
      EventMessage.EventType := MouseMoveEvent;
      send(client.socked, EventMessage, SizeOf(EventMessage), 0);
      SendEvent := NoEvent;
    end;
  end;
