...
procedure TTcpServer.BotFindMove;
begin
...
begin
  ...
  begin
    ...
    msg := StrPas(PAnsichar(@(FReadBuffer)[0]));
    Log.Info(Format('client:%d say: %s', [FSocket, msg]));
    msg := 'this is server';
    StrCopy(@rmsg, PAnsiChar(msg));
    SendData(@rmsg, SizeOf(rmsg));
